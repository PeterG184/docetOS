...
/* Getter for the current check code */
...
/* SVC Handler for wait. Takes a uint32_t called code to check against the check code */
void _svc_OS_wait(_OS_SVC_StackFrame_t const * const stack, uint32_t code) {
	_scheduler->wait_callback((OS_TCB_t *)stack->r0, (uint32_t)stack->r1);
}

/* SVC Handler for notify, also increments the check code */
