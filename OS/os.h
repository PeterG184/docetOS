...
typedef struct {
	...
	void (* wait_callback) (void * const reason, uint32_t code);
...
/* Returns the current check code */
...
/* SVC delegate to wait */
void __svc(OS_SVC_WAIT) OS_wait(void * const reason, uint32_t code);

/* SVC delegate to notify */
